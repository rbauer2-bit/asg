...
Dream 100 Strategy + Cold Email + Belief-to-Buy Sequence
...
INPUT_MD = "client-onboarding/clients/kristen-lemastus-coaching/dream100-cold-email-b2b-sequence_20260304.md"
OUTPUT_DOCX = "client-onboarding/clients/kristen-lemastus-coaching/kristen-lemastus-coaching_dream100-cold-email-b2b-sequence_20260304.docx"
...
def add_cover_page(doc):
    ...
    run = cover_title.add_run('DREAM 100 STRATEGY')
    ...
    r_sub = subtitle.add_run('Cold Email Sequence + Belief-to-Buy Email Sequence')
    r_sub.font.name = 'Calibri'
    r_sub.font.size = Pt(18)
    r_sub.font.color.rgb = ASG_CHARCOAL
    ...
    attorney.paragraph_format.space_after = Pt(36)
...
def parse_and_render(doc, md_path):
    ...
    while i < len(lines):
        ...
        # Skip the original cover page content (first section until first ---)
        if not cover_done:
            if line.strip() == '---':
                cover_done = True
            i += 1
