...
30-Day YouTube Long-Form Script Pack — Family Law Attorney Market
"""
...
INPUT_MD = "client-onboarding/clients/authority-systems-group/outputs/lgd-family-law_youtube-30day-longform-scripts_20260305-COMPLETE.md"
OUTPUT_DOCX = "client-onboarding/clients/authority-systems-group/outputs/lgd-family-law_youtube-30day-longform-scripts_20260305.docx"
...
def add_cover_page(doc):
    ...
    run = cover_title.add_run('30-DAY YOUTUBE LONG-FORM SCRIPT PACK')
    ...
    r = subtitle.add_run('Family Law Attorney Market')
    r.font.name = 'Calibri'
    r.font.size = Pt(22)
    r.font.bold = True
    ...
    niche.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r2 = niche.add_run('30 Fully-Written YouTube Scripts | Belief-to-Buy Architecture | Dopamine Ladder Mirror')
    r2.font.name = 'Calibri'
    r2.font.size = Pt(13)
    r2.font.color.rgb = ASG_CHARCOAL
    niche.paragraph_format.space_after = Pt(48)

    add_horizontal_rule(doc)

    for line in [
        'Produced by: Milo Vance, YouTube Long-Form Script Writer',
...
def add_footer(doc):
    ...
    run = footer_para.add_run(
        'Authority Systems Group™ — Confidential. Prepared exclusively for Family Law Attorney Market.'
    )
    run.font.name = 'Calibri'
    run.font.size = Pt(8)
...
def parse_and_render(doc, md_path):
    ...
    while i < len(lines):
        ...
        # Skip the original cover page header (lines before first ---)
        if not cover_done:
            if line.strip() == '---':
                cover_done = True
            i += 1
