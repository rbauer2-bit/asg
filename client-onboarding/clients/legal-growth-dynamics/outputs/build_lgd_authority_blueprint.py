...
Legal Growth Dynamics | Authority Blueprint™
March 2026
"""
...
INPUT_MD    = os.path.join(BASE_DIR, 'lgd_authority-blueprint_20260302.md')
OUTPUT_DOCX = os.path.join(BASE_DIR, 'lgd_authority-blueprint_20260302.docx')
...
def add_cover_page(doc):
    ...
    r2 = sub.add_run('The Complete Authority System for Family Law Firm Growth')
...
def parse_and_render(doc, md_path):
    ...
    while i < len(lines):
        ...
        # Skip the original cover block (everything up to and including first ---)
        if not cover_done:
            if line.strip() == '---':
                cover_done = True
            i += 1
